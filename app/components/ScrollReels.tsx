...
export default function ScrollReels({ initialChronicles }: ScrollReelsProps) {
  ...
  const loadNextPage = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/getAllChronicles?page=${page + 1}&limit=20`,
        {
          headers,
        }
      );
      ...
      console.log(result,'newChronicles');
  ...
  useEffect(() => {
    if (
      visibleIndex >= chronicles.length - 2 &&
      !loading &&
      !loadingRef.current &&
      hasMore
    ) {
          ...
          {chronicles.map((item, index) => (
            ...
            >
              <div className="relative w-full h-[96vh] md:h-full flex justify-center items-center bg-[#fffff0] text-[#2d2d2d]">
                <div className="absolute w-full bg-[#000] left-0 top-0 flex items-center justify-between px-1 py-3 z-50">
                  <div className="flex items-center gap-3">
                    <div className="relative w-10 h-10 rounded-full overflow-hidden ring-2 ring-[#980000]">
                       <Image
                        src={item?.user?.profilePicture || img1}
                        alt="profile"
                        fill
                    ...
                    <div className="flex flex-col">
                      <Link
                        href={`/author?u=${item?.user?.username}`}
