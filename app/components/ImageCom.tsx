  ...
  const ImageComWrapper = (props: P) => {
    ...
    const contentClass =
      pathname === "/dashboard" || pathname=== "/dashboard/write-chronicle"
        ? `${styles.content} w-full lg:w-[50%] items-start h-full lg:top-40`
        : `${styles.content}`;
    return (
      <div
        className={`${styles.wrapper} h-screen flex justify-center lg:justify-end items-center`}
      >
        <div
          className={`${styles.bg} ${
            !showSecond ? styles.visible : styles.hidden
          }`}
